...
- `moldvision dataset create --name "my-dataset" -c monitor -c keyboard` (use multiple `-c` for multiple classes)
- `moldvision dataset create --name "my-dataset" --classes-file classes.txt` (one class name per line)
...
- You can also use a custom output directory: `moldvision dataset extract-frames -v video1.mp4 -n 100 -o my_frames/`

1c) (Optional) Inspect your datasets:

- `moldvision dataset list`  — table of all datasets under the configured root (UUID, name, date, classes, task, trained)
- `moldvision dataset list --root D:\other_datasets`  — list a specific root
- `moldvision dataset info -d datasets/<UUID>`  — splits image/annotation counts, class names, model status, exports
...
- Merge an external COCO export into `train`:
  - `moldvision dataset import-coco -d datasets/<UUID> --split train --coco-json path\\to\\_annotations.coco.json --images-dir path\\to\\images --mode copy --align-metadata`
- Convert YOLO to a temp COCO folder:
  - `moldvision dataset yolo-to-coco -d datasets/<UUID> --task detect --out-dir datasets/<UUID>/exports/yolo_to_coco_tmp --copy-images`
- Merge the generated temp COCO into `train` (or `valid`):
  - `moldvision dataset import-coco -d datasets/<UUID> --split train --coco-json datasets/<UUID>/exports/yolo_to_coco_tmp/train/_annotations.coco.json --images-dir datasets/<UUID>/exports/yolo_to_coco_tmp/train --mode copy --align-metadata`
...
- **FP16 (Half Precision)**: `moldvision export -d datasets/<UUID> -w ... --format onnx_fp16`
    - Offers ~2x speedup on modern GPUs with minimal accuracy loss. No calibration needed.
- **INT8 Quantization**: `moldvision export -d datasets/<UUID> -w ... --format onnx_quantized`
...
- `moldvision export -d datasets/<UUID> -w datasets/<UUID>/models/checkpoint_best_total.pth --format tensorrt --fp16`
...
- **Include multiple precisions in the bundle**:
  - `moldvision bundle -d datasets/<UUID> -w ... --quantize` (Adds INT8 `model_quantized.onnx`)
  - `moldvision bundle -d datasets/<UUID> -w ... --export onnx_fp16` (Adds FP16 `model_fp16.onnx`)
...
Force a specific backend with `--backend {tensorrt,onnx,pytorch}`, or set a persistent default with `moldvision config set inference-backend onnx`.
