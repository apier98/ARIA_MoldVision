...
def _ctor_best_effort(cls: type, *, num_classes: Optional[int], pretrain_weights: Optional[str]) -> Any:
    kwargs: Dict[str, Any] = {}
    ...
    if task_norm == "seg":
        ...
        for name in candidates:
            ...
            if isinstance(cls, type):
                model = _ctor_best_effort(cls, num_classes=num_classes, pretrain_weights=pretrain_weights)
                return model, name, True

        preview = getattr(rfdetr, "RFDETRSegPreview", None)
        if isinstance(preview, type):
            model = _ctor_best_effort(preview, num_classes=num_classes, pretrain_weights=pretrain_weights)
